...
function getTenDayData(weatherData) {
  ...
  tenDayProps = weatherData.forecast.simpleforecast.forecastday.reduce(
    (array, item) => {
...
function getCurrentWeatherData(weatherData) {
  ...
  currentWeather = {
    ...
    month: 
      weatherData.forecast.simpleforecast.forecastday[0].date.monthname_short,
    day: weatherData.forecast.simpleforecast.forecastday[0].date.day,
    currentTemp: (weatherData.current_observation.temp_f).toFixed(0),
    expectedHigh: 
      weatherData.forecast.simpleforecast.forecastday[0].high.fahrenheit,
    expectedLow: 
      weatherData.forecast.simpleforecast.forecastday[0].low.fahrenheit,
    summary: weatherData.forecast.txt_forecast.forecastday[0].fcttext,
    icon: weatherData.forecast.txt_forecast.forecastday[0].icon_url
  };
